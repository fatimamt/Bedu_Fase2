...
mex <- mutate(mex, NuevosInfectados = c(1, diff(Infectados))) # Nuevos infectados por día
mex <- mutate(mex, NuevosMuertos = c(0, diff(Muertos))) # Nuevos muertos por día

mex <- mutate(mex, Letalidad = round(Muertos/Infectados*100, 1)) # Tasa de letalidad

?lag

mex <- mutate(mex, InfectadoDiaAnterior = lag(Infectados), MDA = lag(Muertos)) # Valores día anterior
mex <- mutate(mex, FactoresCrecimientoInfectados = Infectados/InfectadoDiaAnterior, FCM = Muertos/MDA) # Factores de Crecimiento
mex <- mutate(mex, Dia = 1:dim(mex)[1]) # Días de contingencia

head(mex); tail(mex)

write.csv(mex, file = 'Analisis.csv')
